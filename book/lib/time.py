...
class Time:
    ...
    def _set_noun_time(self):
        np = NounPhrase(self.noun_dict)
        np.build_noun_phrase(self.timestamp)

        # returns ['日曜日', 'にちようび']
        self.noun = np.get_noun_phrase()[0]
...
if __name__ == '__main__':
    from basics import load_from_json, keyed_dictionary
    ...
    timestamp = 'Sunday'
    print(Time(timestamp, nouns).get_time())
